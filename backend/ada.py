...
CHANNELS = 1
# Default (high-quality) sample rates and chunk sizes
SEND_SAMPLE_RATE = 16000
RECEIVE_SAMPLE_RATE = 24000
CHUNK_SIZE = 1024

# Low-power defaults (used when AudioLoop.low_power=True)
LOW_POWER_SEND_RATE = 8000
LOW_POWER_RECEIVE_RATE = 8000
LOW_POWER_CHUNK_SIZE = 2048
...
config = types.LiveConnectConfig(
    ...
    system_instruction="Your name is HIDI, which stands for Hyper Intelligent digital interface.. "
        "You have a witty and charming personality. "
        "Your creator is Ashley, and you address her as 'Ashley', 'Miss' or 'Madam'. "
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE, on_audio_data=None, on_video_frame=None, on_cad_data=None, on_web_data=None, on_transcription=None, on_tool_confirmation=None, on_cad_status=None, on_cad_thought=None, on_project_update=None, on_device_update=None, on_error=None, input_device_index=None, input_device_name=None, output_device_index=None, kasa_agent=None, low_power=False):
        ...
        # Low-power mode reduces sample rates, increases chunk sizes, and can disable video
        self.low_power = low_power
        self.send_rate = LOW_POWER_SEND_RATE if self.low_power else SEND_SAMPLE_RATE
        self.receive_rate = LOW_POWER_RECEIVE_RATE if self.low_power else RECEIVE_SAMPLE_RATE
        self.chunk_size = LOW_POWER_CHUNK_SIZE if self.low_power else CHUNK_SIZE

        if self.low_power and self.video_mode == "camera":
            print("[HIDI] Low-power mode active: disabling camera to save resources.")
    ...
    async def listen_audio(self):
        ...
        if self.input_device_name:
            print(f"[HIDI] Attempting to find input device matching: '{self.input_device_name}'")
            ...
            if resolved_input_device_index is not None:
                print(f"[HIDI] Resolved input device '{self.input_device_name}' to index {resolved_input_device_index} ({best_match})")
            else:
                print(f"[HIDI] Could not find device matching '{self.input_device_name}'. Checking index...")
        ...
        if resolved_input_device_index is None and self.input_device_index is not None:
             try:
                 ...
                 print(f"[HIDI] Requesting Input Device Index: {resolved_input_device_index}")
             except ValueError:
                 print(f"[HIDI] Invalid device index '{self.input_device_index}', reverting to default.")
                 resolved_input_device_index = None

        if resolved_input_device_index is None:
             print("[HIDI] Using Default Input Device")
        ...
        try:
            ...
            print(f"[HIDI] [ERR] Failed to open audio input stream: {e}")
            print("[HIDI] [WARN] Audio features will be disabled. Please check microphone permissions.")
            return
        ...
        while True:
            ...
            try:
                ...
                # 2. VAD Logic for Video — use efficient audioop.rms (C implementation)
                try:
                    rms = audioop.rms(data, 2)
                except Exception:
                    # Fallback to pure-Python RMS if audioop fails
                    count = len(data) // 2
                    if count > 0:
                ...
                if rms > VAD_THRESHOLD:
                    ...
                    if not self._is_speaking:
                        ...
                        print(f"[HIDI] DEBUG] [VAD] Speech Detected (RMS: {rms}). Sending Video Frame.")
                        
                        # Send ONE frame
                        if self._latest_image_payload and self.out_queue:
                            await self.out_queue.put(self._latest_image_payload)
                        else:
                            print(f"[HIDI DEBUG] [VAD] No video frame available to send.")
                    ...
                    if self._is_speaking:
                        if self._silence_start_time is None:
                            ...
                            print(f"[HIDI DEBUG] [VAD] Silence detected. Resetting speech state.")
                            self._is_speaking = False
                            self._silence_start_time = None
    ...
    async def handle_cad_request(self, prompt):
        ...
        if self.project_manager.current_project == "temp":
            ...
            if success:
                ...
                try:
                    ...
                    print(f"[HIDI DEBUG] [ERR] Failed to notify auto-project: {e}")
        ...
        if cad_data:
            print(f"[HIDI DEBUG] [OK] CadAgent returned data successfully.")
            print(f"[HIDI DEBUG] [INFO] Data Check: {len(cad_data.get('vertices', []))} vertices, {len(cad_data.get('edges', []))} edges.")
            
            if self.on_cad_data:
                print(f"[HIDI DEBUG] [SEND] Dispatching data to frontend callback...")
                self.on_cad_data(cad_data)
                print(f"[HIDI DEBUG] [SENT] Dispatch complete.")
            ...
            try:
                ...
                print(f"[HIDI DEBUG] [NOTE] Sent completion notification to model.")
            except Exception as e:
                 print(f"[HIDI DEBUG] [ERR] Failed to send completion notification: {e}")

        else:
            print(f"[HIDI DEBUG] [ERR] CadAgent returned None.")
    ...
    async def handle_write_file(self, path, content):
        print(f"[HIDI DEBUG] [FS] Writing file: '{path}'")
        ...
        if self.project_manager.current_project == "temp":
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            new_project_name = f"Project_{timestamp}"
            print(f"[HIDI DEBUG] [FS] Auto-creating project: {new_project_name}")
        ...
        print(f"[HIDI DEBUG] [FS] Resolved path: '{final_path}'")
        ...
        print(f"[HIDI DEBUG] [FS] Result: {result}")
        try:
             await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
             print(f"[HIDI DEBUG] [ERR] Failed to send fs result: {e}")

    async def handle_read_directory(self, path):
        print(f"[HIDI DEBUG] [FS] Reading directory: '{path}'")
        try:
    ...
    async def handle_read_file(self, path):
        print(f"[HIDI DEBUG] [FS] Reading file: '{path}'")
        try:
            if not os.path.exists(path):
    ...
    async def handle_web_agent_request(self, prompt):
        print(f"[HIDI DEBUG] [WEB] Web Agent Task: '{prompt}'")
        ...
        print(f"[HIDI DEBUG] [WEB] Web Agent Task Returned: {result}")
        ...
        try:
             ...
             print(f"[HIDI DEBUG] [ERR] Failed to send web agent result to model: {e}")
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_output_transcription:
                                    ...
                                    if delta:
                                        # Send to frontend (Streaming)
                                        if self.on_transcription:
                                             self.on_transcription({"sender": "HIDI", "text": delta})
                                        
                                        # Buffer for Logging
                                        if self.chat_buffer["sender"] != "HIDI":
                                            # Flush previous
                                            if self.chat_buffer["sender"] and self.chat_buffer["text"].strip():
                                                self.project_manager.log_chat(self.chat_buffer["sender"], self.chat_buffer["text"])
                                            # Start new
                                            self.chat_buffer = {"sender": "HIDI", "text": delta}
                                        else:
                                            # Append
                                            self.chat_buffer["text"] += delta
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad"]:
                                ...
                                if not confirmation_required:
                                    print(f"[HIDI DEBUG] [TOOL] Permission check: '{fc.name}' -> AUTO-ALLOW")
                                    ...
                                    print(f"[HIDI DEBUG] [STOP] Requesting confirmation for '{fc.name}' (ID: {request_id})")
                                    ...
                                    print(f"[HIDI DEBUG] [CONFIRM] Request {request_id} resolved. Confirmed: {confirmed}")

                                    if not confirmed:
                                        print(f"[HIDI DEBUG] [DENY] Tool call '{fc.name}' denied by user.")
                                ...
                                if fc.name == "generate_cad":
                                    print(f"\n[HIDI DEBUG] --------------------------------------------------")
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call Detected: 'generate_cad'")
                                    print(f"[HIDI DEBUG] [IN] Arguments: prompt='{prompt}'")
                                    ...
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'run_web_agent' with prompt='{prompt}'")
                                    ...
                                    print(f"[HIDI DEBUG] [RESPONSE] Sending function response: {function_response}")
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'write_file' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'read_directory' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'read_file' path='{path}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'create_project' name='{name}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'switch_project' name='{name}'")
                                    ...
                                    if success:
                                        ...
                                        print(f"[HIDI DEBUG] [PROJECT] Sending project context to AI ({len(context)} chars)")
                                        try:
                                            await self.session.send(input=f"System Notification: {msg}\n\n{context}", end_of_turn=False)
                                        except Exception as e:
                                            print(f"[HIDI DEBUG] [ERR] Failed to send project context: {e}")
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": msg}
                                    )
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'list_projects'")
                                    ...
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'list_smart_devices'")
                                    ...
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'discover_printers'")
                                    ...
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'print_stl' STL='{stl_path}' Printer='{printer}'")
                                    ...
                                    printer = fc.args["printer"]
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'get_print_status' Printer='{printer}'")
                                    ...
                                    print(f"[HIDI DEBUG] [TOOL] Tool Call: 'iterate_cad' Prompt='{prompt}'")
                                    ...
                                    if cad_data:
                                        print(f"[HIDI DEBUG] [OK] CadAgent iteration returned data successfully.")
                                        
                                        # Dispatch to frontend
                                        if self.on_cad_data:
                                            print(f"[HIDI DEBUG] [SEND] Dispatching iterated CAD data to frontend...")
                                            self.on_cad_data(cad_data)
                                            print(f"[HIDI DEBUG] [SENT] Dispatch complete.")
                                        ...
                                        print(f"[HIDI DEBUG] [ERR] CadAgent iteration returned None.")
    ...
    async def get_frames(self):
        ...
        # Attempt to reduce camera resolution to lower CPU usage
        try:
            ...
            # Lower target FPS
            await asyncio.to_thread(cap.set, cv2.CAP_PROP_FPS, 5)
        except Exception:
            pass
        while True:
            if self.paused:
            ...
            if frame is None:
                break
            # Throttle frame sending; increase when low_power
            await asyncio.sleep(2.0 if self.low_power else 1.0)
            if self.out_queue:
    ...
    def _get_frame(self, cap):
        ...
        # Reduce thumbnail size to lower CPU and network usage
    ...
    async def run(self, start_message=None):
        ...
        while not self.stop_event.is_set():
            try:
                print(f"[HIDI DEBUG] [CONNECT] Connecting to Gemini Live API...")
                async with (
                    ...
                    # Smaller queue when in low-power mode to limit memory and scheduling
                    self.out_queue = asyncio.Queue(maxsize=(5 if self.low_power else 10))
                    ...
                    if not is_reconnect:
                        if start_message:
                            print(f"[HIDI DEBUG] [INFO] Sending start message: {start_message}")
                        ...
                        print(f"[HIDI DEBUG] [RECONNECT] Connection restored.")
                        # Restore Context
                        print(f"[HIDI DEBUG] [RECONNECT] Fetching recent chat history to restore context...")
                        ...
                        print(f"[HIDI DEBUG] [RECONNECT] Sending restoration context to model...")
                ...
                print(f"[HIDI DEBUG] [STOP] Main loop cancelled.")
                break
                
            except Exception as e:
                # This catches the ExceptionGroup from TaskGroup or direct exceptions
                print(f"[HIDI DEBUG] [ERR] Connection Error: {e}")
                
                if self.stop_event.is_set():
                    break
                
                print(f"[HIDI DEBUG] [RETRY] Reconnecting in {retry_delay} seconds...")
